...
//	frame->SetClientSize(400, 500);
	frame->SetMinSize(wxSize(400, 500)); //Sets the minimum size of the window, Might not be needed thats why its commented out
	frame->SetMaxSize(wxSize(400, 500)); //Sets the maximum size of the window, Might not be needed thats why its commented out
	frame->Center();
	frame->Show(true);
	return true;
}
